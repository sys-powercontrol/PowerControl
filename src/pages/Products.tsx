...
export default function Products({ defaultTab = "Produtos" }: ProductsProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Tabs */}
      <div className="flex flex-wrap items-center gap-2 p-1.5 bg-gray-100/90 rounded-2xl border border-gray-200/80 w-full sm:w-fit">
        {["Produtos", "Categorias", "Marcas"].map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all cursor-pointer ${
              activeTab === tab 
